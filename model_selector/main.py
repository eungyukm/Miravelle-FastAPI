...
def get_nima_score(image_path):
    ...
    print(f"[NIMA 평가] {image_path}: {final_score:.2f}")  # 디버깅 로그 추가
    return final_score
...
기능 설명  
- 프롬프트(키워드)를 입력하고 **Execute** 버튼을 누르면,  
- `generated_3d_models` 폴더에 있는 이미지들 중  
  **프롬프트와의 유사도 점수 + 미적 품질 점수(NIMA) 계산**  
- 최종 점수가 가장 높은 이미지를 반환  
"""